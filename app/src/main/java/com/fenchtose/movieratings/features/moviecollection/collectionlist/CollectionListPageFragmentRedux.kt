...
class CollectionListPageFragmentRedux: BaseFragment() {
...
//                    presenter?.deleteCollection(collection)
    ...
    private fun onCollectionSelected(collection: MovieCollection) {
        if (shouldReturnSelection()) {
            ...
            ResultBus.setResult(CollectionListPageFragment.CollectionListPagePath.SELECTED_COLLECTION, ResultBus.Result.create(collection))
        } else {
...
//                    presenter?.share()
    ...
    private fun shouldReturnSelection(): Boolean {
        path?.let {
            return (it as CollectionListPageFragment.CollectionListPagePath).returnSelection
        }
...
class CollectionListPath(val returnSelection: Boolean): RouterPath<CollectionListPageFragmentRedux>() {
    companion object {
        const val SELECTED_COLLECTION = "selected_collection"
    }

    override fun createFragmentInstance() = CollectionListPageFragmentRedux()
